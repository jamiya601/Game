...
public class Game {
	...
	public static void main(String[] args) {
		...
		gui = new GUI();
		if(gui == null) {
			System.out.println("GUI failed to run.");
			return;
		}
		currentRoom = World.buildWorld();
		if(currentRoom == null) {
			System.out.println("World failed to build.");
			return;
		}
		print(currentRoom);
	...
	public static void processCommand(String command) {
		...
		case "d":
			...
			if(followingRoom.getLock()) { // checks if following room is locked 
				Game.print("This room is locked."); // prints out the room is locked
			} else {
		...
		case "use":
			if(words.length == 2) { // "look" and item's name
				...
				if(item != null) {
					...
					Game.print("There's no item to use.");
				}
			}
			break;
		...
		case "open":
			if(words.length == 2) {
				...
				if (item != null) {
					...
					Game.print("There's no item to open.");
				}
			}
			break;
		...
		case "take": // takes an item in the room and add it to your inventory and removes it from the room
			if(words.length == 2) { // "take" and item's name
				...
				Game.print("A " + name + " have been added to your inventory.");
				} else {
					Game.print("There's no item here.");
				}
			}
			break;
		...
		case "i": // checks if inventory is empty, if not, prints out items in your inventory
			if(command.equals("i")) {
				...
				Game.print("There is nothing in your inventory.");
				}
				else {
		...
		case "look": // to look at an item in the room
			if(words.length == 2) { // "look" and item's name
				...
				if(item != null) { // prints description of item if item is found in the room
					Game.print("There is: " + item.getDescription());
				} else {
					Game.print(name + " is not here or in your inventory.");
				}
			}
			break;
			
		default:
			Game.print("Invalid input");
		}
	}
